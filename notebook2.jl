...
md"""
# Optimal data-driven numerical schemes for implicit turbulence modelling
"""

# ╔═╡ 64b763b5-3e17-469f-b6c5-69c109529977
md"Before we start, let's load libraries that we will use throughout the tutorial"

# ╔═╡ 19a15fe3-63c6-405c-8d2e-5e25816608ab
md"
## 1. The 1D viscous Burgers' equation
...
function f_conv(um, ui, up, upp, κ) # κ-scheme + Rusanov
...
let
	...
	@info a_periodic[end+1] == a_periodic[1]
end
...
let # check E=1/2
	...
	E = energy(u_IC)
	@info("E = $E")
	@info isapprox(E, 1/2; rtol=1/100)
end
...
md"And then we define the plotting function using [Makie](https://docs.makie.org/stable/). These plotting functions and parameters are hidden in the next cell becauses they are not relevant for the present purpose. Also the function to get direct numerical simulation (DNS) reference data that has been already generated is located there."

# ╔═╡ 5e72aa37-59b6-4b12-87ed-68e65eb096cd
...
md"With the grid in place, and the function to plot the solution state hidden in the next cell, we can finally display the initial random condition"
...
md"## 2. Modelling subgrid scales using numerical dissipation
...
function opt(; verbose=false)
	...
	result = optimize(f, κ_min, κ_max, κ₀, optimizer, options; 
...
manifest_format = "2.0"
project_hash = "6fd4e1129bde498a6a3363c025f9e6921dc6bc06"
